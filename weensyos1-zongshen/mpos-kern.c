...
interrupt(registers_t *reg)
{
	...
	case INT_SYS_EXIT:
		...
		current->p_exit_status = current->p_registers.reg_eax;
		        //change who depend on it to P_RUNNABLE
		int j;
		for ( j = 1; j < NPROCS; j++) {
	              if(proc_array[j].wait_pid == current->p_pid)
...
do_fork(process_t *parent)
{
    ...
    if(i != NPROCS)
    {
       ...
       proc_array[i].p_registers=current->p_registers;//for struct we can just assign
       copy_stack(&proc_array[i],current);//set &proc_array[i].reg_esp=stack pointer
