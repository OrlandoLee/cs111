...
task_t *start_download(task_t *tracker_task, const char *filename)
{
    ...
    //EXERCISE 2A
    //strcpy has buffer overflow issues
    //we changed it to strncpy(char *dest, const char *src, size_t n)
...
static void task_upload(task_t *t)
{
    ...
    {
        error("Filename too long in request");
        goto exit;
    }
    ...
    int j;
	for(j = 0; j < FILENAMESIZ && t->filename[j] != '\0'; j++)
	{
		if(t->filename[j] == '/')
		{
...
static void task_evil_download(task_t *t, task_t *tracker_task)
{
	...
	if (t->peer_fd == -1) {
     ...
     //Exercise 3B long name attack to overflow buffer
	...
	strncpy(t->filename,  "this is a long stringthis is a long stringthis is a long stringthis is a long stringthis is a long stringthis is a long stringthis is a long stringthis is a long stringthis is a long stringthis is a long stringthis is a long stringthis is a long stringthis is a long stringthis is a long stringthis is a long stringthis is a long stringthis is a long stringthis is a long stringthis is a long stringthis is a long string ", FILENAMESIZ - 1);
	// Then put that filename 2 times, so it's over FILENAMESIZ and not null terminated
