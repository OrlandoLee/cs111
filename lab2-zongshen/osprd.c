...
MODULE_AUTHOR("Chang Zhao and Zongsheng Li");
...
static void osprd_process_request(osprd_info_t *d, struct request *req)
{
	...
	// Consider the 'req->sector'( sector_t sector; /* next sector to submit */), 'req->current_nr_sectors'(/* no. of sectors left to submit in the current segment */), and
	// 'req->buffer' members /* kaddr of the current segment*/, and the rq_data_dir() function/*((rq)->cmd_flags & 1)*/.
    ...
    {
        eprintk("execced ramdisk size\n");
    }
    else if(rq_data_dir(req)==READ)//from ramdisk
    {
